...
public class PhilosopherDeadlock extends Thread{
        ...
        public PhilosopherDeadlock(int name, Fork leftFork, Fork rightFork) {
            super();
...
//        }
        ...
        public void run() {
            while (amountEaten < 51) {
                System.out.println("P"+name+" is thinking");
                checkIfLeftForkAvailable();
                checkIfRightForkAvailable();
        ...
        private synchronized void checkIfRightForkAvailable() {
//            System.out.println(this.name + " checking if right fork available");
            while (!forks.get(1).isAvailable()){
                try {
                    wait();
                }
...
//            hasLeftFork = false;
        ...
        private synchronized void checkIfLeftForkAvailable() {
//            System.out.println(this.name + " checking if left fork available");
            while (!forks.get(0).isAvailable()){
                try {
                    wait();
                }
                catch (InterruptedException ignored) {}
